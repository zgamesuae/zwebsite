...
class Get_a_quote extends \App\Controllers\BaseController{
    ...
    public function checkrequest(){
        ...
        $errors["err_msg"]="Please enter more than 3 configuration element";
        ...
        if(sizeof($errors) == 0){
            ...
            if($email_sent)
                return redirect()->to(base_url()."/get_a_quote/request_sent/1/".$data["name"]);
            else
                return redirect()->to(base_url()."/get_a_quote/request_sent/0/".$data["name"]);
        }
    ...
    public function email_pcgamer_query($idata){
        if($to = $this->systemModel->get_website_settings()->email_2){
            $subject = 'PC GAMER - SAUDI | Quote request from '.$idata['infos']["name"];
            $message = view('Quoterequest_email', $idata);
            $email = \Config\Services::email();
            $email->setTo($to);
            $email->setCC("yahia@3gelectronics.biz");
