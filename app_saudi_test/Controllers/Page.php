...
class Page extends BaseController
{
    ...
    public function paymentSuccess($oid , $flag = true){
        ...
        // Reactivate the User Session
        if(!is_null($session->get("userLoggedin")))
        $user_id = $session->get("userLoggedin");

        else if(isset($_GET["user"])){

            $user_id = $_GET["user"];
            $session->set("userLoggedin", $user_id);

            // verify Paytab request signature 
            // if(!$this->paytabModel->paytab_verify_request(filter_input_array(INPUT_POST)));
            // return redirect()->to(
        ...
        $payment_id = (isset($_POST["tranRef"])) ? $_POST["tranRef"] : null;
            ...
            if(isset($data["payment_method"])){
                ...
                if($user_id){
                    ...
                    if($payment_method == "Tabby payment"){
                        ...
                        if($this->orderModel->cart_has_digital_codes($user_id))
                        return $this->checkout("Sorry! Tabby Payement Not Available for Gift Cards orders");
                        ...
                        // var_dump($scoring);
                        // var_dump(json_encode($tabby_data));
                        // die();
                    ...
                    switch ($payment_method) {
                        ...
                        case 'Cash on delivery':
                            ...
                            $this->orderModel->order_zatca_complicance_check($order_details["order"]["order_id"]);
                        ...
                        case 'Online payment':
                            ...
                            if($data["wallet_use"] == "Yes" && false){
                                ...
                                // ######## PayTab Payment Start #########
                                    // Prapare Payload
                                    $payload = $this->paytabModel->structure_data($order_details);
                                    $response = $this->paytabModel->paytab_initiate_payment_page($payload , $user_id);
                                    // var_dump($response);die();
                                if($response["status"])
                                // return $this->payment_page($response["url"]);
        ...
        if($res){
            ...
            // check if there is a bundle product in the cart if true check the quantity if it is max 1
                foreach ($res as $key => $value) {
                    ...
                    if($value->quantity <= 0 || empty($value->quantity) || !$productModel->is_in_stock($value->product_id , $value->quantity)){
                        // $session->setFlashData("")
                        return $this->cart();
                    }
                }
    ...
    public function getSearchData(){
        ...
        if ($this->request->getMethod() == "post"){
            ...
            $data["_offerModel"] = $this->offerModel;
    ...
    public function update_preorders(){
        ...
        $req = "update products set pre_order_enabled='No' where pre_order_enabled ='Yes' and release_date <= :r_date: and release_date IS NOT NULL and release_date<>'0000-00-00'";
