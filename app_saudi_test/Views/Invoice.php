<?php 
  ...
  if(count(@$uri->getSegments())>1){
    ...
    // Get The QR Code if exists
    $QR_file = APPPATH."Libraries/ZATCA/tmp/QRCodes/{$order_details["order_id"]}_QR";
    $QR = (file_exists($QR_file)) ? file_get_contents($QR_file) : false;
    $total = 0;

    // var_dump(date("d M Y H:i:s" , strtotime($order_details["created_at"])));die();
...
<body>
  <header>
    ...
    <h1>فاتورة ضريبية مبسطة</br>Simplified Tax Invoice </h1>
    <address>
      <p>
        <?php echo $settings[0]->business_name; ?>
      </p>
      <p>
      ...
      <br>
      <p>
        Phone: <?php echo $settings[0]->phone;?>
      </p>
      <p>
        Email: <?php echo $settings[0]->email;?>
      </p>
      <br>
    </address>
    ...
    <address style="float:right; text-align: right;">
      <p>
        CR Certificate N°/رقم شهادة التسجيل <br>1010847540
      </p>
      <p>
        VAT Registration N°/رقم التسجيل الضريبي <br>311497332600003
      </p>
      <br>
    </address>  
  ...
  <article>
    ...
    <address>
      ...
      <p>
        <?php echo $order_details["street"] ?>
      </p>
      <p>
        <?php echo $orderModel->get_city_name($order_details["city"])->title ?>
      </p>
      <p>
        <?php echo $order_details["phone"] ?>
      </p>
      <p>
    ...
    <table class="meta">
      ...
      <tr>
        ...
        <td><span>
            <?php echo date("d M Y H:i:s" , strtotime($order_details["created_at"]));?>
          </span></td>
      </tr>
      <tr>
      ...
      <tr>
        <td>
          <address>
            ...
            <p>
              <?php echo $order_details["apartment_house"] ?>
            </p>
            <p>
        ...
        <td style="text-align:center">
          <!-- <img height="150px" width="150px" src="<?php echo $QR ?>" alt=""> -->
      ...
      <?php endif; ?>
        ...
        <!-- TAXABLE AMOUNT -->
				<tr>
					...
					<td><span><?php $taxable = bcdiv($order_details["sub_total"] / 1.15 , 1 , 2); echo $taxable ?></span> <span data-prefix><?php echo CURRENCY ?></span></td>
				</tr>
        <!-- TAXABLE AMOUNT -->

        <!-- CHARGES AMOUNT -->
        <?php $taxable_charges = bcdiv($order_charges["total_charges"] /1.15 , 1 , 2) ?>
        ...
        <tr>
            ...
            <td><span ><?php echo bcdiv($charge["price"] / 1.15 , 1 , 2) ?></span> <span data-prefix><?php echo CURRENCY ?></span></td>
        </tr>
        <?php endforeach; ?>
        <!-- CHARGES AMOUNT -->
        ...
        <?php
          ...
          if((int)$order_details["coupon_discount"] > 0): 
            $taxable_discount = bcdiv($order_details["coupon_discount"] / 1.15 , 1 , 2);
        ?>
				<tr>
        ...
        foreach($order_details["offers"] as $offer):
          if((int)$offer->amount > 0): 
            $taxable_discount += bcdiv($offer->amount / 1.15 , 1 , 2);
        ?>
				<tr>
					<th><span >خصم / Discount <?php if($offer->discount_type == "Percentage") echo " ({$offer->value}%)"  ?></span></th>
					<td>
            <span> <?php echo bcdiv($offer->amount / 1.15 , 1 , 2) ?> </span> <span data-prefix> <?php echo CURRENCY ?></span>
          </td>
				</tr>
        <?php 
          endif; 
        ...
        <tr>
            ...
            <td>
              <span><?php echo bcdiv(($taxable + $taxable_charges - $taxable_discount) * 0.15 , 1 , 2) ?> </span><span data-prefix> <?php echo CURRENCY ?> </span>
            </td>
        </tr>
        <!-- VAT AMOUNT -->
  ...
  <aside>
    ...
    <!-- <div>
      <p>ZamZam games reserves the right to change these Terms of Use at any time, effective immediately upon posting on
        the Site. Please check this page of the Site periodically.</p>
    </div> -->
  </aside>
</body>
