...
class OrderModel extends Model{
    ...
    public function cart_product_price($cartid , $user_id = null , $vat=true){
        ...
        if($this->cart_has_coupon_applied($user_id)["applied"]){
            ...
            foreach ($cart as $key => $product) {
                ...
                if($product->coupon_type == "Percentage"){
                    $product_price = $this->cart_product_price($product->id , $user_id);
    ...
    public function total_cart($user_id,$brand=null,$category=null){
        ...
        $sql = "
        select 
        products.price,products.discount_percentage,cart.product_id,cart.* 
        from cart inner join products on cart.product_id=products.product_id 
        where cart.user_id='".$user_id."' and products.status='Active'
        ";
        ...
        if($res){
            ...
            foreach($res as $key => $value){
                ...
                $data_price = $this->cart_product_price($value->id , $user_id);
    ...
    public function get_user_cart($user_id){
        ...
        $req = "select product_image.image,a_cart.* from (select products.name,products.price,products.sku,products.type,cart.* from cart inner join products on cart.product_id=products.product_id where user_id='".$user_id."' and products.status='Active' group by cart.id) as a_cart left join product_image on a_cart.product_id=product_image.product group by a_cart.id";
     $res = $this->userModel->customQuery($req);
    ...
    public function create_order_products($user_id,$cart,$order){
        ...
        foreach($cart as $product){
            ...
            if(!empty($product->pre_order_enabled) && trim($product->pre_order_enabled) !== "" && $product->pre_order_enabled == "Yes")
            $_products["preorder_status"] = "Pre-order";
    ...
    public function prepare_order_details($user_address , $payment_method , $user_id = null , $order_id = null){
        ...
        if(sizeof($offers) > 0){
            ...
            foreach ($offers as $offer) {
                ...
                //     $saved_prizes = $offerModel->get_cart_product_prizes($user_id);
                //     $order_offer += [
                //         "prizes" => (sizeof($saved_prizes) > 0) ? $saved_prizes : $offerModel->_select_offer_valid_prize($offer[0] , $user_id)
                //     ];
                // }
    ...
    public function create_order($order){
        ...
        if(sizeof($order["order"]) > 0 && (isset($order["order"]["email"]) && !empty($order["order"]["email"]))){
            ...
            // Save order details
            $order_id = $this->userModel->do_action("orders", "", "", "insert", $order["order"], "");
            ...
            if(sizeof($prizes) > 0){
                ...
                array_map(function($prize) use($order_id , $userModel){
                    ...
                    //         $this->userModel->do_action("order_prizes", "", "", "insert", $order_prize_product, "");
            ...
            array_map(function($offer) use (&$userModel){
                unset($offer["prizes"]);
                $this->userModel->do_action("order_offers", "", "", "insert", $offer, "");
            } , $order["offers"]);
    ...
    public function cart_has_digital_codes($user_id){
        ...
        where products.type='6' and products.status='Active' and cart.user_id='$user_id'
        ";
        $res = $this->userModel->customQuery($req);
    ...
    public function online_payment_order_process($oid , $user_id , $payment_id = null){
        ...
        $paytabModel = model("App\Models\Paytab");
        ...
        if ($oid && $user_id) {
            ...
            switch ($payment_txn[0]->paymethod) {
                ...
                default:
                    # code...
                    // Telr Check Transaction
                    // $results = $this->telr_check_payment($payment_txn[0]->ref); //return transaction infos array

                    // ¨PayTab Check Transaction
                    $results = $paytabModel->paytab_check_payment($payment_txn[0]->ref);
                    if($results && sizeof($results) > 0 && $results["status"] == "Paid"){
            ...
            if($order_process){
                ...
                if(sizeof($digital_codes) > 0 && true){
                ...
                $this->order_zatca_complicance_check($order_details["order"]["order_id"]);
    ...
    public function order_zatca_complicance_check($order_id){
        ...
        // var_dump($order , $order["offers"]);die();
        ...
        array_map(function($product) use(&$line_items){
            $line_items[] = [
                ...
                'quantity' => $product->quantity - $product->free_quantity,
                'tax_exclusive_price' => round(bcdiv($product->product_original_price / 1.15 , 1 , 3) , 2),
                'VAT_percent' => 0.15,
                // 'other_taxes' => [
                //     ['percent_amount' => 1]
                // ],
                'discounts' => ($product->discount_percentage > 0 || $product->discount_amount > 0) ? [['amount' => round(bcdiv($product->product_original_price / 1.15 , 1 , 2 ) , 2) - round(bcdiv($product->product_price / 1.15 , 1 ,2) , 2), 'reason' => 'Discount']] : [],
            ];
        ...
        // MAP Offers cart discount information to the ZATCA System Data Structure
        if(sizeof($order["offers"]) > 0){
            array_map(function($offer)use(&$discounts){
                if($offer->value > 0)
                $discounts [] = [
                    'type' => $offer->discount_type,
                    'value'=> $offer->value,
                    'amount' => round(bcdiv($offer->amount / 1.15 , 1 , 2) , 2),
                    'reason' => $offer->offer_title,
                    'VAT_percent' => 0.15
                ];
            } , $order["offers"]);
        };
