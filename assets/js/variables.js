...
function get_attribute_options(attribute_id , option_id , requested_attributes , parent){
    $.get(base_url+"/variations/update_attrbite_sections" , {"attribute" : attribute_id , "option" : option_id , "req_attributes" : requested_attributes , "parent" : parent} , function(data){
...
function get_product_variation_price(variation , parent){
    $.get(base_url+"/variations/update_variable_price" , {"variation" : variation , "parent" : parent} , function(data){
        data = JSON.parse(data)
        if(data.price !== null){
            let buy_btn = $(".products_details form.products_add_to_cart button[type='submit']")
            if(data.offer_price !== null){
                $("div.card-subtitle").html("<span> "+data.price+" <span> "+currency+" </span> </span>")
                $("p.offer-price").html("<span style='font-size: inherit;font-weight:inherit'>"+data.offer_price+"</span><span>"+currency+"</span>")
            }
            else{
                $("div.card-subtitle").html("")
                $("p.offer-price").html("<span style='font-size: inherit;font-weight:inherit'>"+data.price+"</span><span>"+currency+"</span>")
            }

            $(buy_btn).html((data.is_preorder == true) ? "Pre-order" : "Add To Cart")
            tabby_product_page.price = data.price
            new TabbyPromo(tabby_product_page)
            

            $("form.products_add_to_cart input[name='product_image']").val(function(index , value){
                if(data.image !== null)
                $(".loaded_images").html(data.image)
            })
...
$(document).ready(function(){
    ...
    $(".product_variations").on("click" , ".var_option.active" , function(){
        ...
        if($(this).parents("ul").find(".var_option.selected").hasClass("ybr")){
            ...
            $(this).addClass("selected ybr h-100")
        }
