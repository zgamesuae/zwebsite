...
class Page extends BaseController
{
    ...
    public function paymentSuccess($oid , $flag = true){
        ...
        $bool = $this->orderModel->online_payment_order_process($oid , $user_id , $payment_id);
        ...
        if($this->request->getMethod() == "post"){
            ...
            if(isset($data["payment_method"])){
                ...
                if($user_id){
                    ...
                    $order_products = $this->orderModel->create_order_products($user_id , $cart , $order_details["order_id"]);
        

                    // If payment method is Tabby, check Operation faisability status
                    if($payment_method == "Tabby payment"){

                        $tabby_data = $this->tabbyModel->structure_data($cart , $order_details["charges"] , $order_details["sub_total"] , $order_details["coupon_discount"] , $user_address , $user_id , false);
                    ...
                    switch ($payment_method) {
                        ...
                        case 'Cash on devlivery':
                            ...
                            if ($data["wallet_use"] == "Yes" && false) {
                                ...
                                if ($cbal[0]->total) {
                                    if ($cbal[0]->total >= $order_details["total"]) {
                                        $oud["payment_status"] = "Paid";
                                        $oud["wallet_used_amount"] = $order_details["total"];
                                        $odetail = $this->userModel->do_action(
                                            "orders",
                            ...
                            // save order products
                            $this->orderModel->save_order_products($order_products);

                            // save charges
                            $this->orderModel->save_order_charges($user_id , $order_details["order_id"] ,$cart_charges);
                            
                            // Clear cart
                            $this->orderModel->clear_cart();

                            // ZATCA Compliance Check
                            $this->orderModel->order_zatca_complicance_check($order_details["order_id"]);
                            ...
                            return redirect()->to( site_url("/order-success?ref=" . base64_encode($order_details["order_id"])) );
                            break;
                                ...
                                if ($cbal[0]->total) {
                                    ...
                                    if ($cbal[0]->total >= $order_details["total"]) {
                                        ...
                                        if ($wallet) {
                                            ...
                                            if ($to = $order_details["email"]) {
                                                $subject ="Your order was successfully submitted. Order Id " . $order_details["order_id"];
                                                $message = view("BookingEmail", ["orders" => $order_details , "order_products" => $order_products , "order_charges" =>  $cart_charges]);
                                                $email = \Config\Services::email();
                                                $email->setTo($to);
                                                $email->setFrom(
                                            ...
                                            return redirect()->to(
                                                site_url(
                                                    "/order-success?ref=" .
                                                        base64_encode($order_details["order_id"])
                                                )
                                            );
                                        }
                                    ...
                                    // Pay a part of the amount with current balance
                                        ...
                                        // ######## Telr Payment Start #########
                                        $telr = $this->orderModel->telr_pay($order_details);
                                        if($telr["status"])
                        ...
                        case 'Tabby payment':
                            ...
                            $tabby_data = ($tabby_data) ?? $this->tabbyModel->structure_data($cart , $order_details["charges"] , $order_details["sub_total"] , $order_details["coupon_discount"] , $user_address , $user_id , false);
            ...
            $this->checkout("Please select a payment method");
    ...
    public function checkout($msg_er = null)
    {
        ...
        if($res){
            ...
            // check if there is a bundle product in the cart if true check the quantity if it is max 1
                foreach ($res as $key => $value) {
                    ...
                    if($value->order_interval !== "Unlimited" && !$value->order_interval == null){
                        $user_email= $userModel->get_user_email($user_id);
                        if($user_email)
            ...
            if(sizeof($p_max_order_qty) > 0){
                ...
                echo $this->header();
                echo view("Checkout" , array("msg" => $msg_er));
    ...
    public function buyNowSubmitForm()
    {
        ...
        if ($this->request->getMethod() == "post") {
            ...
            if ( $this->request->getVar("quantity") > 0 && $this->request->getVar("quantity") != "" && $msg == "" && $productModel->is_in_stock($p["product_id"] , $this->request->getVar("quantity"))) {
                ...
                $p["discount_percentage"]= $productModel->get_discounted_percentage($p["product_id"]);
    ...
    public function updateprice(){
        ...
        if ($this->request->getMethod() == "post") {
            ...
            $price = $userModel->customQuery($req);

            // $additional_price = $userModel->customQuery( "select additional_price from bundle_opt where id='" . $this->request->getVar("bundle_opt_id") . "'"
            $additional_price = $userModel->customQuery( "select sum(additional_price) as additional_price from bundle_opt where FIND_IN_SET(id , '".implode("," , $this->request->getVar("bundle_opt_id"))."')");
