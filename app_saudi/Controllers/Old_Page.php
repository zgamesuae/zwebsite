 ...
 if($oid=base64_decode($oid)){
  ...
  if($results){
    ...
    if($to=$odrr[0]->email){
     ...
     unset($od[0]->coupon_value);  unset($od[0]->invoice);  unset($od[0]->updated_at);   
     $edata['orders']=$od[0];
   ...
   if($res){
     if($res[0]->coupon_type=="specific"){
       ...
       if($spec){
         ...
         if(@$user_id){
           $sql="select *,cart.gift_wrapping as gw from cart 
...
if(@$user_id=$session->get('userLoggedin')){
 ...
 $sql="select *,cart.pre_order_enabled as pre_order_enabled,cart.pre_order_before_payment_percentage as pre_order_before_payment_percentage,cart.discount_percentage as dp,cart.gift_wrapping as gw,cart.assemble_professionally_price as app from cart
 inner join products on cart.product_id=products.product_id
 where cart.user_id='$user_id'";
 $cart=$this->userModel->customQuery($sql); 
 if($cart){
   ...
   foreach($cart as $k=>$v){
    ...
    $ins['product_name']=$v->name;
     @$ins['sku']=@$v->sku;
    if($v->dp > 0 ){
      $ins['product_price']=$v->price - (($v->dp*$v->price)/100);
    } else {
    ...
    if($v->dp > 0 ){
      if($v->app==0 ||$v->app>0){
       $ptotal=$ptotal+(($v->price - ($v->dp*$v->price)/100)  + $v->app + $gw_price ) *$v->quantity;
     }else {
       $ptotal=$ptotal+(($v->price - ($v->dp*$v->price)/100)   + $gw_price ) *$v->quantity;
     }
   } else {
     if($v->app==0 || $v->app>0){
      $app=$v->app;   
...
//############# SEND EMIAL END 
     ...
     $params = array(
      ...
      'ivp_store'   => STOREID,
      'ivp_authkey' => AUTHKEY,
      'ivp_amount'  => $idata['total']-$cbal[0]->total,
      'ivp_currency'=> 'AED',
      'ivp_test' => PGTEST,
...
inner join   related_products on products.product_id=related_products.related_product
where related_products.product_id='$id' AND status='Active'  ";
$data['sproducts']=$this->userModel->customQuery($sql); 
if($data['sproducts']){}else{
