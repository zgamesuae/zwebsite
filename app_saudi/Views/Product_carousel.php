                ...
                <div class="col-12 p-0 m-0" style="height:<?php if(isset($section_division) && sizeof($section_division) > 3): echo "auto"; else: echo "auto"; endif; ?>; overflow:hidden; position:relative">
                    <?php if($countdown): ?>
                        <div style="position: absolute; top:0px; right:0px;" class="col-12 col-lg-4 p-0">
                            <?php echo view("products/Countdown" , ["end_date" => "2023-11-13T00:10:00" , "count_down_title" => "OFFER STARTS IN"]); ?>
                        </div>
                    <?php endif; ?>
                    ...
                    if($list){
                      foreach($list as $k=>$v){ 
                        ...
                        if($productModel->get_discounted_percentage($v->product_id) > 0){
                            $price = ($v->discount_rounded == "Yes") ? round(bcdiv($v->price - ($v->discount_percentage*$v->price)/100, 1, 2)) : bcdiv($v->price - ($v->discount_percentage*$v->price)/100, 1, 2) ;
                        }
                        ?>
                        <div class="item" <?php content_from_right() ?>>
                            <?php echo view("products/Product_box" , ["v" => $v , "product_image" => $product_image , "hr" => false]); ?>
                        </div>
                    <?php 
                        // endif;
                        }
