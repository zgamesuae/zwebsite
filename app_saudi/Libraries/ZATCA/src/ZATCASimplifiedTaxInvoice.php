...
class ZATCASimplifiedTaxInvoice
{
    ...
    {
        $populated_template = require APPPATH . '/Libraries/ZATCA/src/templates/simplified_tax_invoice_template.php';
        ...
        $parseLineItems = $this->parseLineItems($invoice['line_items'] , $invoice["charges"] , $invoice["discounts"]);
        $populated_template = str_replace('PARSE_LINE_ITEMS', $parseLineItems, $populated_template);
        // file_put_contents(APPPATH."/Libraries/ZATCA/tmp/error_invoice.xml" , $populated_template);die();
        // var_dump($populated_template);die();
    ...
    {
        $populated_template = require APPPATH . 'Libraries/ZATCA/src/templates/invoice_billing_reference_template.php';
    ...
    public function getInvoiceHash(DOMDocument $invoice_xml): string
    {
        ...
        // Save Invoice Hash
        file_put_contents(APPPATH."/Libraries/ZATCA/tmp/LIH.txt" , base64_encode($hash));
    ...
    public function generateQR(DOMDocument $invoice_xml, string $digital_signature, $public_key, $signature, string $invoice_hash , string $invoice_serial_number)
    {
        ...
        // var_dump("{$issue_date}-{$issue_time}" , $formatted_datetime);
        ...
        $this->QRrender(base64_encode($qr_tlv), [] , APPPATH."/Libraries/ZATCA/tmp/QRCodes/{$invoice_serial_number}_QR");

        return base64_encode($qr_tlv);
    }

    public function QRrender(string $qrcode_64 , array $options = [], string $file = null): string
    {
        $options = new QROptions($options);
        return (new QRCode($options))->render($qrcode_64, $file);
    }
    ...
    {
        $populated_template = require APPPATH . 'Libraries/ZATCA/src/templates/ubl_signature_signed_properties_for_signing_template.php';
    ...
    {
        $populated_template = require APPPATH . 'Libraries/ZATCA/src/templates/ubl_signature_signed_properties_template.php';
    ...
    public function defaultUBLExtensions(string $invoice_hash, string $signed_properties_hash, string $digital_signature, string $cleanUpCertificateString, string $ubl_signature_signed_properties_xml_string): string
    {
        ...
        $populated_template = require APPPATH . 'Libraries/ZATCA/src/templates/ubl_signature.php';
    ...
    private function parseLineItems(array $line_items , array $charges = [] , array $discounts = [])
    {
        // BT-110
        $total_taxes = 0;
        $total_discount = 0;
        list($cacAllowanceCharges, $total_charges, $cacAllowanceDiscount, $total_discount) = $this->construct_DL_AllowanceTotals($charges , $discounts);
        ...
        $total_taxes = number_format((($total_subtotal + $total_charges - $total_discount) * 0.15) , 2 , '.' , '');
        ...
        /**
         * Populate the Invoice level Chareges
         * Populate the Invoice level discounts
         */

         $invoice_level_allowance_template = require APPPATH . '/Libraries/ZATCA/src/templates/invoice_level_allowance_charges_template.php';
         $allowance_section = $invoice_level_allowance_template["allowance_section"];
         $allowance_details_charge = $invoice_level_allowance_template["allowance_details_charge"];
         $allowance_details_discount = $invoice_level_allowance_template["allowance_details_discount"];
         $allowance_line = "";
        // Charges
        foreach ($cacAllowanceCharges as $charge) {
            # code...
            $allowance_copy = str_replace("__Allowance_Number" , $charge["cbc:ID"] , $allowance_details_charge);
            $allowance_copy = str_replace("__Allowance_indicator" , $charge["cbc:ChargeIndicator"] , $allowance_copy);
            $allowance_copy = str_replace("___AllowanceChargeReason" , $charge["cbc:AllowanceChargeReason"] , $allowance_copy);
            $allowance_copy = str_replace("__Allowance_Reason_Code" , $charge["cbc:AllowanceChargeReasonCode"] , $allowance_copy);
            $allowance_copy = str_replace("__Charge_Amount" , $charge["cbc:Amount"]["#text"] , $allowance_copy);

            $allowance_copy = str_replace("__Allowance_Detail" , $allowance_copy , $allowance_section);

            $allowance_line .= $allowance_copy;
        }

        // Discounts
        foreach ($cacAllowanceDiscount as $discount) {
            # code...
            $allowance_copy = str_replace("__Allowance_Number" , $discount["cbc:ID"] , $allowance_details_discount);
            $allowance_copy = str_replace("__Allowance_indicator" , $discount["cbc:ChargeIndicator"] , $allowance_copy);
            $allowance_copy = str_replace("___AllowanceChargeReason" , $discount["cbc:AllowanceChargeReason"] , $allowance_copy);
            $allowance_copy = str_replace("__Charge_Amount" , $discount["cbc:Amount"]["#text"] , $allowance_copy);
         ...
         * set invoice lines
         */
        $tax_total_template = require APPPATH . 'Libraries/ZATCA/src/templates/tax_total_template.php';

        $item_lines = $this->constructTaxTotal($line_items , $total_charges ,$total_discount);
         ...
         */
        $legal_monetary_total_template = require APPPATH . 'Libraries/ZATCA/src/templates/legal_monetary_total_template.php';

        $constructLegalMonetaryTotal = $this->constructLegalMonetaryTotal($total_subtotal, $total_taxes , $total_charges , $total_discount);
        ...
        $legal_monetary_total_template = str_replace('_ChargeTotalAmount', $constructLegalMonetaryTotal['cbc:ChargeTotalAmount']['#text'], $legal_monetary_total_template);
         ...
         * set invoice lines
         */
        $invoice_line_template = require_once APPPATH . 'Libraries/ZATCA/src/templates/invoice_line_template.php';
        ...
        foreach ($invoice_line_items as $item) {
            ...
            $invoice_line_template_copy = str_replace('__ItemtNetPrice', $item['cac:Price']["cbc:PriceAmount"]["#text"], $invoice_line_template_copy);

            $invoice_line .= $invoice_line_template_copy;
        }

        return $allowance_line . $tax_total_template . $legal_monetary_total_template . $invoice_line;
    }
    ...
    private function constructLineItemTotals($line_item): array
    {
        ...
        $line_item_subtotal = ($line_item['tax_exclusive_price'] - $line_item_total_discounts) * $line_item['quantity'];

        // Calc total taxes
        // BR-KSA-DEC-02
        $line_item_total_taxes = $line_item_total_taxes + ($line_item_subtotal * $line_item['VAT_percent']);

        // Construct Classified Tax Categories For Other Taxes
        if(isset($line_item['other_taxes'])){
            array_map(function ($tax) use (&$line_item_total_taxes, $line_item_subtotal, &$cacClassifiedTaxCategories) {
                $line_item_total_taxes = $line_item_total_taxes + ($line_item_subtotal * floatval($tax['percent_amount']));
    ...
    private function construct_DL_AllowanceTotals(array $charges = [] , array $discounts = []): array
    {
        [$total_charges , $total_discount , $charges_total_taxes , $i] = [0 , 0 , 0 , 1];
        $cacAllowanceCharges = [];
        $cacAllowanceDiscount = [];

        // Calc Charges
        array_map(function ($charge) use (&$total_charges, &$cacAllowanceCharges , &$i) {
            $total_charges += $charge['amount'];
            $cacAllowanceCharges[] = [
                'cbc:ID' => $i,
                'cbc:ChargeIndicator' => 'true',
                'cbc:AllowanceChargeReason' => $charge['reason'],
                'cbc:AllowanceChargeReasonCode' => $charge['code'],
                'cbc:Amount' => [
                    '@_currencyID' => 'SAR',
                    // BR-KSA-F-04
                    '#text' => number_format($charge['amount'], 2, '.', '')
                ]
            ];
            $i++;
        ...
        // Calc Discount
        array_map(function ($discount) use (&$total_discount, &$cacAllowanceDiscount , &$i) {
    ...
    private function constructLegalMonetaryTotal(float $total_subtotal, float $total_taxes , float $total_charges , float $total_discount)
    {
        return [
            // BR-DEC-09
            'cbc:LineExtensionAmount' => [
            ...
            'cbc:TaxExclusiveAmount' => [
                '@_currencyID' => 'SAR',
                '#text' => (number_format(($total_subtotal + $total_charges - $total_discount), 2, '.', ''))
            ],
            // BR-DEC-14, BT-112
            'cbc:TaxInclusiveAmount' => [
                '@_currencyID' => 'SAR',
                '#text' => number_format(($total_subtotal + $total_charges - $total_discount) + $total_taxes, 2, '.', '')
            ],
            ...
            'cbc:ChargeTotalAmount' => [
                '@_currencyID' => 'SAR',
                '#text' => number_format($total_charges , 2, '.', '')
            ],
            'cbc:PrepaidAmount' => [
                '@_currencyID' => 'SAR',
                '#text' => 0
    ...
    private function constructTaxTotal(array $line_items , float $total_charges , float $total_discounts)
    {
        $cacTaxSubtotal = [];
        ...
        $taxes_total = 0;
        $total_taxable = 0;
        array_map(function ($line_item) use (&$taxes_total , &$total_taxable) {
            ...
            $taxable_amount = ($line_item['tax_exclusive_price'] - ($total_line_item_discount ?? 0)) * $line_item['quantity'];
        ...
        // Calculate Total taxable including invoice charges and discounts
        $total_taxable += ($taxable_amount + $total_charges) - $total_discounts;
        $taxes_total = 0.15 * ((float)$total_taxable);

        // BT-110
        $taxes_total = number_format($taxes_total, 2, '.', '');
        $addTaxSubtotal($total_taxable, $taxes_total, 0.15);
