...
class Category extends Model
{
    ...
    public function get_parent_categories($offset , $limit){
        ...
        $parents = array_filter($this->categories , function($category){ return $category["parent_id"] == "0"; });
        $parents = array_slice($parents , $offset , $limit);
        return $parents;

        
        $req="select category_id,category_name,category_name_arabic from master_category where parent_id='0'and status='Active' and show_in_menu = 'Yes' order by precedence LIMIT ".$offset.",".$limit;
        $res=$this->userModel->customQuery($req);
        if($res){
            return $res;
        }
    ...
    public function get_category_brands($id){
        ...
        $req="select brand.*,count(brand.id) as occurence from products inner join brand on products.brand = brand.id where ".$subcat_query." and products.status = 'Active' and products.product_nature<>'Variation' and brand.status='Active' and brand.image<>'' group by brand.id order by occurence desc limit 0,8";
        $res = $this->userModel->customQuery($req);
        if($res && sizeof($res) > 0)
        return $res;
