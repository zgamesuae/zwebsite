<style>
.ws-ns-subscribe button[type='submit']{
    position: absolute;
    top:50%; 
    right:0px; 
...
<?php if(false): ?>
<div class="container-fluid newletter_subscription pt-4 pb-4 text-capitalize" id="ns_sub_bar" <?php content_from_right() ?>>
...
<div class="row">
    <div class="col-12 mt-2">
        <div class="heading">
            <p class="text-white <?php text_from_right() ?> mb-1"><?php echo lg_get_text("lg_15")?></p>
        </div>
    </div>
    <div class="col-12 mt-2">
        <?php if(@$flashData['ns-sub-success'] && true){?>
        ...
        <?php } else if($flashData["ns-sub-failure"]):?>
            <p style="font-size:.9rem;margin-bottom: 0; padding-bottom: 10px; color: #fff; font-weight: 500;color:#ff6666"><?php echo $flashData['ns-sub-failure'];?></p>
        <?php endif; ?>
        <form method="post" class="d-flex ws-ns-subscribe" action="<?php echo base_url();?>/page/newsletter">
            <div class="form-group col-12 p-0" style="position: relative">
                <button type="submit" class="col-4"><?php echo lg_get_text("lg_16") ?></button>
                <input class="form-control col-12" type="email" placeholder="<?php echo lg_get_text("lg_17") ?>" required name="email">
            </div>
            <!-- <button type="submit" class="btn btn-primary"><?php echo lg_get_text("lg_16") ?></button> -->
        </form>
    </div>
