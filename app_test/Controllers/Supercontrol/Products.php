...
class Products extends \App\Controllers\BaseController {
    ...
    public function bulkUpload(){
        ...
        $recognized_field = array(
            ...
            "set_as_new" =>                             array("required_fo_update"=>false , "required_for_creation"=>false),
    ...
    public function upload_other() {
        ...
        if (!$input) {
            ...
            if ($img) {
                $type = $_FILES["file"]["type"];
                $tmp = $_FILES["file"]["tmp_name"];
                $b = move_uploaded_file($tmp , ROOTPATH . "/assets/others/" . $img->getName());
    ...
    public function index() {
        ...
        $crud->displayAs('discount_percentage','Discount');
        $crud->displayAs('offer_start_date','Offer Start');
        $crud->displayAs('offer_end_date','Offer End');
        ...
        // Show Parent SKU if it is Variation
        $crud->callbackColumn('parent', function ($value, $row) {
            $html = "";
            if($row->product_nature == "Variation"){
                $html = $this->productModel->get_product_sku_from_id($value);
            }
            return $html;
        });
    ...
    public function ezpin(){
        ...
        $crud->setRelation("type" , "type", "title");
        $crud->setRelation("google_category" , "google_product_categories", "category");
        $crud->setRelation("regions" , "ezpin_regions", "title");
        $crud->displayAs('type','Product type');
        $crud->displayAs('set_as_new','Is new');
    ...
    public function exchange_rates(){
      ...
      $crud->setTable('ezpin_currency_exchange');
      $crud->displayAs('from_code','From Currency');
      $crud->displayAs('to_code','To Currency');
      $crud->setSubject('Currency Rate', 'Currency Rates');
      $crud->columns(['from_code' , 'to_code' , 'rate']);

      $crud->unsetAdd();
      $crud->unsetDelete();

      $crud->defaultOrdering('from_code', 'asc');
      $output = $crud->render();
      return $this->_example_output($output);
