...
class OfferModel extends Model{
    ...
    // Get Running Offers
    public function _get_valid_offers($offers , $product_id = null , $cart = null): Array{
        ...
        $cart_condition_complies = function($cart , $condition) use ($orderModel , &$valid_products):Array{
            ...
            foreach($cart as $product_cart){
                $not_discounted = ((int)$product_cart->discount_percentage == 0 || is_null($product_cart->discount_percentage)) && is_null($product_cart->coupon_code);
                ...
                if($not_discounted && $not_preorder && $has_product_list && in_array($product_cart->product_id , $condition->product_list)){
                    ...
                    $valid_products[$product_cart->product_id] = ["id" => $product_cart->product_id , "cart_id" => $product_cart->id ,  "name" => $product_cart->name , "qty" => intval($product_cart->quantity) , "price" => $product_cart->price , "offer_condition" => $condition->description , "offer_condition_qty" => $condition->on_product_qty];
                }
        ...
        if($offers && sizeof($offers) > 0){
            array_map(function($offer) use (&$valid_offers , &$cart_condition_complies , &$on_product , &$on_cart , &$product_id , &$cart , &$valid_products){
                ...
                switch (true) {
                    case $on_product:
                        ...
                        # Check if the offer is valid fro the given products
                    ...
                    case $on_cart:
                        ...
                        if($offer_valid){
                            ...
                            if($this->_apply_offer($offer) == "Discount" && $offer->discount_type == "Amount"){
                                $qts = [];
                                array_map(function($product)use(&$qts){ $qts[] = intdiv($product["qty"] , $product["offer_condition_qty"]);} , $offer->valid_cart_products);
    ...
    public function get_offers_conditions_filters($offers = null){
        ...
        $offers = (is_null($offers)) ? array_filter($this->offers_list , function($offer){
            ...
            if(($offer->level == 'Cart' && !is_null($offer->get_qty)) || $offer->level == 'Product' || ($offer->level == 'Cart' && is_null($offer->get_qty) && is_null($offer->discount_value) && sizeof($offer->prizes) > 0))
            return true;
            else
            return false;
        }) : $offers;
