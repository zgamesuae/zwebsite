...
class Spintowin extends BaseController{
    ...
    public function spin_wheel($order_id){
        ...
        $is_registred = $userModel->is_registred($user_id);
        $spinned = $this->wheel_spined($user_id,$order_id);
        
        $order_date = ($orderModel->get_last_order_date($user_id)) ? new \DateTime($orderModel->get_last_order_date($user_id) , $timezone) : false;
        $start_d = new \DateTime("2022-11-1 00:00:01" , $timezone);
        $end_d = new \DateTime("2022-11-31 00:00:01" , $timezone);
    ...
    public function spin_result($order_id){
        ...
        if($this->spin_wheel($order_id)){
            ...
            $res = $userModel->do_action("user_spined_wheel" , "" , "" , "insert" , ["user_id"=>$user_id,"order_id"=>$order_id,"prize"=>$prizes[$item]] , "");
    ...
    public function get_prize($order_id){
        ...
        if($this->request->getMethod() == "get"){
            ...
            if($user_id !== false && $this->wheel_spined($user_id,$order_id) && $orderModel->user_has_order($user_id , $order_id)){
                $customer = $userModel->get_user($user_id);
