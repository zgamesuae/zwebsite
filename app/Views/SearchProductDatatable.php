...
if($product){
	foreach($product as $k=>$v){
		...
		<div class="p-10px col-md-3  col-sm-6 col-6 mb-3">
			...
			<?php if(false): ?>
			<div class="product_box shadow-none bg-white rounded overflow-hidden d-flex flex-column justify-content-between" style="height: 100%">
        		...
        		<div class="promotion_sticker get_right_sticker">
        		    <div class="<?php if(!is_null($offer->get_qty)) echo "get_sticker_tag"; else echo "prize_sticker_tag"; ?> p-1">
						<?php $prize_title = (isset($offer->prize_title)) ? $offer->prize_title : "a prize"; ?>
        		        <p class="p-0 m-0 text-center" style="font-weight: bold; line-height: 16px;">Buy <?php echo $offer->buy_qty ?> Get <?php if(!is_null($offer->get_qty)) echo $offer->get_qty; else echo $prize_title  ?></p>
        		    </div>
        		</div>
        		<?php 
        		endif; 
