...
if(isset($filters["keyword"])){
    $search_keyword = $p_model->search_product($filters["keyword"] , false);
    $score_limit = (sizeof(explode(" ", $filters["keyword"])) > 1) ? sizeof(explode(" ", $filters["keyword"])) : 0 ;
    $pre_sql = "select * from (select ".$search_keyword["score"].",";
    $post_sql = ") as sub_query where match_score >= $score_limit ";
}

// By type
$sql = "type_id,title,arabic_title from type inner join products on type.type_id=products.type ";
$sql.=$p_model->filter($filters);
$sql = (isset($filters["keyword"])) ? $pre_sql.$sql." $post_sql group by type_id order by title asc" : "select $sql group by type_id order by title asc";
// echo($sql);
// die();
...
// By brand
$sql = "id,title,arabic_title from brand inner join products on brand.id=products.brand ";
$sql.= $p_model->filter($filters);
$sql = (isset($filters["keyword"])) ? $pre_sql.$sql." $post_sql group by id order by title asc" : "select $sql group by id order by title asc";
// echo($sql);die();
@$brand = $userModel->customQuery($sql);

// By suitable for
$sql = " id,title,arabic_title  from suitable_for inner join products on suitable_for.id=products.suitable_for ";
$sql.=$p_model->filter($filters);
$sql = (isset($filters["keyword"])) ? $pre_sql.$sql." $post_sql group by id" : "select $sql group by id";
@$suitable_for = $userModel->customQuery($sql);

// By Regions
$sql = " id,title from ezpin_regions inner join products on ezpin_regions.id=products.regions ";
$sql.=$p_model->filter($filters);
$sql = (isset($filters["keyword"])) ? $pre_sql.$sql." $post_sql " : "select $sql ";
$sql .= "group by id order by title asc";
@$regions = $userModel->customQuery($sql);
// echo($sql);die();


// By genre
$sql = " id,title,arabic_title  from color inner join products on  FIND_IN_SET(color.id , products.color) ";
$sql.=$p_model->filter($filters);
if($data["genre"])
...
// By Age
$sql = " id,title  from age inner join products on age.id=products.age ";
$sql.=$p_model->filter($filters);
$sql = (isset($filters["keyword"])) ? $pre_sql.$sql." $post_sql group by id" : "select $sql group by id";
...
// By category
$sql = "category_id,category_name from master_category inner join products on master_category.category_id=products.category";
$sql.=$p_model->filter($filters);
$sql = (isset($filters["keyword"])) ? $pre_sql.$sql." $post_sql group by category_id" : "select $sql group by category_id";
...
<div class="container-fluid bg-lightt p-0" <?php echo content_from_right() ?>>
    ...
    <div class="container outer_container_div pb-5 col-12 col-lg-10">
        
        <?php 
        if($uri->getSegment(2) == "offers" && !isset($_GET["offer_cdn"]) && sizeof($GLOBALS["offerModel"]->offers_list) > 0)
        echo view("offers/Offers_list"); 
        ?>
            ...
            <div class="px-2 col-lg-3 mb-2 ws-pf-container">
                <div class="product_filter col-12 px-3 px-md-0 rounded bg-white shadow-none ">
                    <form action="<?php echo base_url();?>/page/getSearchData" method="post" id="getsearchform">
                        ...
                        <?php if(isset($data['offer_cdn'])): ?>
                        <input name="offer_cdn" id="offer_cdn" type="hidden" value="<?php echo @@$data['offer_cdn'];?>">
                        <?php endif; ?>
                        ...
                        <?php if(isset($data["ws-search-category"])) ?>
                        <input name="ws-search-category" id="ws-search-category" type="hidden" value="<?php echo @$data["ws-search-category"];?>">
                                              ...
                                              foreach($LeftCat as $k=>$v){
                                                ...
                                                if ($cid2=$v->category_id) {
                                                  ...
                                                  $sql=$sql." ) ";
                                                           ...
                                                           $sql="select category_id,category_name from master_category inner join products on master_category.category_id=products.category where FIND_IN_SET('$cid2', products.category) group  group by master_category.category_id";
                                                         }
                                                }
                                                else{
                                                   $sql="select category_id,category_name from master_category inner join products on master_category.category_id=products.category";
                        ...
                        <div class="filter_category">
                            <div class="titie_heasder">
                                <h3><?php echo lg_get_text("lg_393") ?></h3>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">
                                    <path fill="none" d="M0 0h24v24H0z"></path>
                                    <path d="M12 13.172l4.95-4.95 1.414 1.414L12 16 5.636 9.636 7.05 8.222z"></path>
                                </svg>
                            </div>
                            <div class="cate_data_body" style="display:<?php if(@@$data['regions']) echo 'block';else echo 'none'?>;">
                                <ul class="f-el-l">
                                    <?php
                                    ...
                                    <li class="<?php text_from_right() ?>">
                                        <label class="checkbox_label">
                                            <?php lg_put_text($v->title , $v->title); ?>
                                            <input type="checkbox" <?php if( in_array($v->id , explode("," , $data["regions"]))): echo "checked"; endif;?> class="JagatFilterInput" id="regions" value="<?php echo $v->id?>" name="regions">
                                            <span class="checkmark"></span>
                                        </label>
                                    </li>
                                    <?php } ?>
                        ...
                        <!-- offer filter -->
                        <?php
                        $sql="select discount_percentage,round(discount_percentage) as discount_round from products ";
                        $sql .= $p_model->filter($filters);
                        ...
                        <div class="filter_category">
                            <div class="titie_heasder">
                                <h3><?php echo lg_get_text("lg_379") ?></h3>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">
                                    <path fill="none" d="M0 0h24v24H0z"></path>
                                    <path d="M12 13.172l4.95-4.95 1.414 1.414L12 16 5.636 9.636 7.05 8.222z"></path>
                                </svg>
                            </div>
                            <div class="cate_data_body" style="display:<?php if(@@$data['stock-status']) echo 'block';else echo 'none'?>;">
                                <ul>
                                    <li class="<?php text_from_right() ?>">
                                        <label class="checkbox_label"><?php echo lg_get_text("lg_380") ?>
                                            <input <?php if(@@$data['stock-status']=="in") echo 'checked';?>
                                                type="checkbox" class="JagatFilterInput" value="in" name="stock_status">
                                            <span class="checkmark"></span>
                                        </label>
                                    </li>
                                    <li class="<?php text_from_right() ?>">
                                        <label class="checkbox_label"><?php echo lg_get_text("lg_55") ?>
                                            <input <?php if(@@$data['stock-status']=="out") echo 'checked';?>
                                                type="checkbox" class="JagatFilterInput" value="out" name="stock_status">
                                            <span class="checkmark"></span>
                                        </label>
                                    </li>
                                </ul>
