...
class Orders extends \App\Controllers\BaseController{
    ...
    public function products(){
        ...
        $uri = service("uri");
        $uri3 = (sizeof($uri->getSegments()) > 3) ? $uri->getSegment(4) : null;
        ...
        $crud->setRelation("product_id", "products", "sku");
    ...
    public function preorder_products(){
        ...
        $crud->setCsrfTokenName(csrf_token());
        $crud->setCsrfTokenValue(csrf_hash());
        $crud->setSubject("Pre-order product", "Pre-order products");
        $crud->setTable("order_products");
        $crud->setRelation("product_id", "products", "release_date");
        $crud->where([
        ...
        $crud->fieldType("order_id", "hidden");
        $crud->unsetFields(['created_at','updated_at']);
        $crud->unsetSearchColumns(['release_date' , 'product_id' , 'product_image']);
        ...
        $crud->displayAs('product_id', 'Release Date');
        ...
        $crud->callbackColumn('product_image', function ($value, $row) {
            if (!empty($row->product_image) && trim($row->product_image) !== "") {
                $html = '<img class="m-0 p-0" width="80" src="' . base_url() . '/assets/uploads/' . $row->product_image . '">';
            } else {
                $html = '<img class="m-0 p-0" width="80" src="' . base_url() . '/assets/uploads/noimg.png">';
            }
            //   return "<a href='/supercontrol/Products/productImage/" . $row->product_id."' >$html</a>";
            return "<a target='blank' href='".base_url()."/assets/uploads/$row->product_image'>$html</a>";
        });

        $crud->callbackColumn("order_id", function ($value, $row) {
            return "<a href='" .
                base_url() .
                "/invoice/" .
                $row->order_id .
                "' target='blank'>$row->order_id (invoice)</a>";
        });
    ...
    public function index(){
        ...
        // {
        //     $date = new \dateTime($value,new \dateTimeZone("Asia/Dubai"));
    ...
    public function ezpin_orders(){   
      ...
      $this->ezpinModel->update_pending_orders();
      $crud->setCsrfTokenName(csrf_token());
      $crud->setCsrfTokenValue(csrf_hash());
      ...
      $crud->displayAs('ez_order_id','EZpin Order ID');
      // $crud->where(["type" => "6" ]);
      $crud->columns(["order_id" , "ez_order_id" , "product_id" , "reference_code" , "share_link" , "price" , "status_text" , "created_at" , "updated_at"]);
