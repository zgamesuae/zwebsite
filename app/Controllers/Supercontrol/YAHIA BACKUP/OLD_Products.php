...
class Products extends \App\Controllers\BaseController {
    ...
    // jagat bulk function
    public function bulkUpload___jold() {
        $data = [];
        helper(['form', 'url']);
        if ($this->request->getMethod() == "post") {
            $input = $this->validate(['file' => ['uploaded[file]']]);
                        ...
                        foreach ($arra as $k => $v) {
                            ...
                            if ($insertData) {
                                ...
                                if ($sku2 = $v[17]) {
                                    ...
                                    if ($prExisit) {
                                        if ($insertData['name']) $this->userModel->do_action('products', $sku2, 'sku', 'update', $insertData, '');
    ...
    public function yahia() {
        ...
        if (is_int((int)$this->request->getVar('stock_qty'))) {
            if (!$this->userModel->do_action('products', $this->request->getVar('product_id'), 'product_id', 'update', array("available_stock" => $this->request->getVar('stock_qty')), '')) $msg[0] = "Something went wrong";
        } else {
