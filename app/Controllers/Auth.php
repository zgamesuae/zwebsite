...
class Auth extends BaseController
{
    ...
    public $config = [
        'callback' => 'https://zgames.ae/auth/social_login',
        'providers' => [
            ...
            'Facebook' => ['enabled' => true, 'keys' => ['id' => '253156953778102', 'secret' => '5998784c844d11d9969643eb649b1deb']],
    ...
    public function update_cart_after_login($session_id , $user_id){
        ...
        // Update Guest Cart product Prizes
        $req = "select id from cart_product_prizes where user_id='$session_id'";
        $res = $this->userModel->customQuery($req);
        if(!is_null($res) && sizeof($res) > 0){
            array_map(function($cart_prize)use(&$user_id){
                $this->userModel->do_action("cart_product_prizes" , $cart_prize->id , "id" , "update" , ["user_id" => $user_id] , "");
            } , $res);
        }
    } 
