...
class Sitemap extends BaseController{
    ...
    public function getlastsitemap(){
        $filepath = ROOTPATH."/ae_sitemap/sitemap.xml";
    ...
    public function constructsitemap(){
        ...
        $file = ROOTPATH."ae_sitemap/sitemap.xml";
    ...
    public function construct_xml_feed(){
        $file = ROOTPATH."ae_sitemap/productfeed.xml";
    ...
    public function product_xml_dom($id , $link){
        ...
        $element .= '
            ...
            <lastmod>'.(new \DateTime($this->productModel->getlastmodified($id) , new \DateTimeZone(TIME_ZONE)))->format("Y-m-d").'</lastmod>
    ...
    public function category_xml_dom($id , $link){
        ...
        $element .= '
            ...
            <lastmod>'.(new \DateTime($this->categoryModel->getlastmodified($id) , new \DateTimeZone(TIME_ZONE)))->format("Y-m-d").'</lastmod>
            <changefreq>daily</changefreq>
    ...
    public function product_feed_xml_dom($id , $link){
        ...
        if($product){
            ...
            // Filter label by Pre-order Status 
            $pre_order = ($product->pre_order_enabled == "Yes") ? "Pre-order" : "Released";
            $element .= "<g:custom_label_0>".htmlspecialchars($pre_order)."</g:custom_label_0>";
            ...
            $discount = $this->productModel->get_discounted_percentage($this->offerModel->offers_list , $id);
            if($discount["discount_amount"] > 0){
                $element .= "<g:sale_price>".$discount["new_price"]." ".CURRENCY."</g:sale_price>";
