...
// $routes->add('buy-tekken-8','Page::productList/tekken-8-preorder-1702306301');

if(true){
  $urls=$cat->categories_urls();
  foreach($urls as $key => $value){
    if($value !== "playstation" && $value !== "xbox" && $value !== "nintendo-switch" && $value !== "pc-gaming" && $value !== "digital-codes-and-gift-cards"){
      $routes->add($value,'Page::productList/'.$key);
    }
    
    else{
      switch (true) {
        case (in_array($value , ["nintendo-switch" , "pc-gaming" , "digital-codes-and-gift-cards"])):
          # code...
...
//  $routes->add('cr/(:any)' , 'tournament/Tournament::cr/$1');
 ...
 $routes->add('tournament/fc24' , 'Tournament/Tournament::fc_24');
//  $routes->add('skull-and-bones-treasure-quest' , 'Tournament/Tournament');
