...
class ProductModel extends Model
{
    ...
    public $type_segments = [
        "video-games" =>["5"],
        "accessories" =>["7","26","27","28","29","30","31","33","34","37","63","62","64","67","72","76","93"],
        "pc-and-consoles" =>["18","36","45","77","82","83","84","89","92"],
        "merchandize" =>["46","47","50","51","54","55","58","60","61","79","80","88","91","94","95","96","98","104","105","106","107","108","110","111","120","121","122"],
    ];
    ...
    public function filter($filters){
        ...
        foreach ($filters as $key => $value) {
            ...
            if($filter[$key]){
                ...
                if($i==0){
                    $sql.=" where products.status='Active' and ";
                    $i++;
                }
                    ...
                    switch (true) {
                        case ($key == 'show-offer'):
                            ...
                            foreach ($offers_filters as $c_filter) {
                                ...
                                $c_b = ($has_brands && !isset($filters["brand"])) || ($has_brands && isset($filters["brand"]) && sizeof(array_intersect($c_filter["brands"] , explode("," , $filter["brand"]))) > 0);
    ...
    public function get_discounted_percentage($offers_list , $id){
        ...
        if($product){
            ...
            if(($discount_cond1 || $discount_cond2) && !is_null($product[0]->discount_type)){
                ...
                $discount["discount_amount"] = ($product[0]->discount_type == "percentage") ? bcdiv(($product[0]->price * $product[0]->discount_percentage / 100) , 1 , 2) : $product[0]->discount_percentage;
            }
    ...
    public function total_cart(){
        ...
        $req = "select count(id) as totalcart from cart inner join products on cart.product_id=products.product_id where user_id='".$user_id."' and products.status='Active'";
        $res = $this->userModel->customQuery($req);
        if(!is_null($res) && sizeof($res) > 0){
    ...
    public function product_filter_query($filter){
        ...
        if(isset($filter["groupby"]) && in_array($filter["groupby"] , ["brand" , "type" , "price"])){
