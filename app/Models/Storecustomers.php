...
class Storecustomers extends Model {
    ...
    public function get_store_cities($country = null){
        $req = "select city from stores ";
        $req .= (!is_null($country)) ? "where country='$country' group by city order by city desc" : "group by city order by country desc,city";
        $res = $this->userModel->customQuery($req);
    ...
    public function get_store_countries(){
        $req = "select country from stores order by country Asc";
        $res = $this->userModel->customQuery($req);
        if($res && !is_null($res))
        return $res;
